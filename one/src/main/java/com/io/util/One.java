...
public class One {
	...
	public static void main(String[] args) {
		 ...
		 // System.out.println(s3[i].concat("  L")); }
		  
		  }
		  
		  String s4 = s2.toLowerCase().replaceAll("\\s+", "!");
		...
		System.out.println(map2.values().stream().skip(3).findFirst().get());

		map2.forEach((i, l) -> {

			String trimmer = l.toLowerCase().trim().replaceAll("[^a-z_]", "");
		...
		map2.keySet().forEach(k->System.out.println(k));
		  ...
		  String sn = " uS34 aP8 %# AGYt e487436024 4186!^1~) haf3^dfD+d^^ &&!*`**$3@!!Y^$z faF44 35@%785 =63ADd^&!*apI)Daea";

		System.out.println(sn.replaceAll("[^*]", ""));
		  
		System.out.println(sn.replaceAll("[^\\D]",""));
		...
		//  System.out.println(s2.replace("\\D", " "));
		 ...
		 // System.out.println(alphabets);
		  ...
		  String st = "dfdfaereregfddvfdbrwwtqfa";
